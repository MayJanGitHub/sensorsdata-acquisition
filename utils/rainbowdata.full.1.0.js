...
kit.buildData = function (p) {
  var _ = rb._;
  var data = {
    distinct_id: rb.store.getDistinctId(),
    identities: rb.store._state.identities,
    lib: {
      $lib: rb.lib_name,
      $lib_method: 'code',
      $lib_version: rb.lib_version
    },
    properties: {}
  };
  ...
  _.extend(data, rb.store.getUnionId(), p);
  ...
  if (!p.type || p.type.slice(0, 7) !== 'profile') {
    ...
    data.properties = _.extend({}, _.info.properties, rb.store.getProps(), _.info.currentProps, data.properties);
    if (p.type === 'track') {
...
function encodeTrackData(data) {
  ...
  var dataStr = rb._.urlBase64Encode(data);
  return encodeURIComponent(dataStr);
}

function setPublicPProperties(data) {
  if (data && data.properties) {
    var refPage = rb._.getRefPage();
    var pageInfo = rb._.getCurrentPageInfo();
    var propertiesMap = {
    ...
    if (rb.para.preset_properties.url_path === true) {
...
mergeStorageData.getData = function (callback) {
  wx.getStorage({
    key: rb.para.storage_prepare_data_key,
    complete: function (res) {
      var queue = res.data && rb._.isArray(res.data) ? res.data : [];
...
mergeStorageData.deleteAesData = function (queue) {
  ...
  if (queue_len > 0) {
    for (var i = 0; i < queue_len; i++) {
      if (rb._.isObject(queue[i])) {
        arr.push(queue[i]);
      }
    }
    rb.store.mem.mdata = arr.concat(rb.store.mem.mdata);
  }
};
...
var sendStrategy = {
  ...
  onAppHide: function () {
    if (rb.para.batch_send) {
      this.batchSend();
    }
  },
  send: function (data) {
    this.dataHasChange = true;
    if (rb.store.mem.getLength() >= 500) {
      rb._.logger.info('数据量存储过大，有异常');
      rb.store.mem.mdata.shift();
    }

    data = kit.processData(data);
    if (data) {
      rb.store.mem.add(data);
    }
    if (rb.store.mem.getLength() >= rb.para.batch_send.max_length) {
      this.batchSend();
    }
  },
  wxrequest: function (option) {
    if (rb._.isArray(option.data) && option.data.length > 0) {
      var data = kit.batchTrackData(option.data);
      var timestamp = new Date().getTime();
      var app_id = rb.para.app_id;
      var app_secret = rb.para.app_secret;
      var dataStr = `app_id=${app_id}&app_secret=${app_secret}&timestamp=${timestamp}&data=${data}`;
      rb._.wxrequest({
        url: rb.para.server_url,
        method: 'POST',
        dataType: 'text',
        data: 'data_list=' + data +
          '&app_id=' +
  ...
  batchWrite: function () {
    ...
    if (this.dataHasChange) {
      ...
      if (this.syncStorage) {
        rb._.setStorageSync(rb.para.storage_prepare_data_key, rb.store.mem.mdata);
      }
    }
  },
  batchInterval: function () {
    ...
    function loopSend() {
      setTimeout(function () {
        _this.batchSend();
        loopSend();
      }, rb.para.batch_send.send_timeout * Math.pow(2, _this.failTime));
    }
    loopWrite();
    loopSend();
  }
...
function onceSend(data) {
  ...
  var encodeData = kit.encodeTrackData(data);
  var timestamp = new Date().getTime();
  var app_id = rb.para.app_id;
  var app_secret = rb.para.app_secret;
  var dataStr = `app_id=${app_id}&app_secret=${app_secret}&timestamp=${timestamp}&data=${encodeData}`;
  rb._.wxrequest({
    url: rb.para.server_url + '?data=' + encodeData + '&app_id=' + app_id,
    method: 'GET',
    header: {
      timestamp: timestamp,
      signature: _.md5(dataStr),
...
rbEvent.send = function (p, callback) {
  if (!rb.para.server_url) {
    rb._.logger.info('error: server_url 不能为空');
    return false;
  }

  if (rb.current_scene && rb.current_scene === 1154 && !rb.para.preset_events.moments_page) {
    return false;
  }

  var data = rb.kit.buildData(p);
  if (data) {
    rbEvent.debug(data);
    rb.events.emit('send', data);
    if (rb.para.batch_send) {
    ...
    rb._.logger.info('error: 数据异常 ' + data);
  }
};

rbEvent.debug = function (data) {
  rb._.logger.info(data);
};
  ...
  if (meta.plugin.uninitialized_list.length > 0) {
    for (var temp in meta.plugin.uninitialized_list) {
      ...
      if (plugin_item && plugin_item.target && typeof plugin_item.target.init === 'function') {
        plugin_item.target.init(rb, plugin_item.para);
      }
    }
...
function checkPrivacyStatus() {
  ...
  if (global && global.rainbow_data_pre_config) {
    is_compliance_enabled = global.rainbow_data_pre_config.is_compliance_enabled ? global.rainbow_data_pre_config.is_compliance_enabled : false;
  }
...
function monitorApp() {
  wx.onAppShow(function (para) {
    if (!meta.life_state.app_launched) {
      ...
      rb.autoTrackCustom.appLaunch(option);
    }
    rb.autoTrackCustom.appShow(para);
  });

  wx.onAppHide(function () {
    rb.autoTrackCustom.appHide();
  });
}
...
function mpProxy(option, method, identifier) {
  var newFunc = rb.autoTrackCustom[identifier];
  if (option[method]) {
    var oldFunc = option[method];
    option[method] = function () {
      if (!rb.para.autoTrackIsFirst || (isObject(rb.para.autoTrackIsFirst) && !rb.para.autoTrackIsFirst[identifier])) {
        oldFunc.apply(this, arguments);
        newFunc.apply(this, arguments);
      } else if (rb.para.autoTrackIsFirst === true || (isObject(rb.para.autoTrackIsFirst) && rb.para.autoTrackIsFirst[identifier])) {
        newFunc.apply(this, arguments);
        oldFunc.apply(this, arguments);
      }
    };
...
function clickTrack(events) {
  ...
  if (isObject(rb.para.framework) && isObject(rb.para.framework.taro) && !rb.para.framework.taro.createApp) {
  ...
  if (type && isClick(type)) {
    if (rb.para.preset_events && rb.para.preset_events.collect_element && rb.para.preset_events.collect_element(arguments[0]) === false) {
      return false;
    }
    prop['$url_path'] = rb._.getCurrentPath();
    rb._.setPageRefData(prop);
    prop = rb._.extend(prop, event_prop);
    rb.track('$MPClick', prop);
  }
}
...
function clickProxy(option, method) {
  ...
  option[method] = function () {
    ...
    if (isObject(args)) {
      if (rb.para.preset_events.defer_track) {
        setTimeout(function () {
          clickTrack(args);
        }, 0);
      } else {
...
function monitorClick(option) {
  var methods = [];
  if (rb.para.autoTrack && rb.para.autoTrack.mpClick) {
...
function monitorHooks(option) {
  if (rb.para.autoTrack && rb.para.autoTrack.pageLeave) {
  ...
  if (typeof option.onShareAppMessage === 'function') {
    rb.autoTrackCustom.pageShare(option);
  }
  if (typeof option.onShareTimeline === 'function') {
    rb.autoTrackCustom.pageShareTimeline(option);
  }
}


rb.kit = kit;
rb.mergeStorageData = mergeStorageData;
rb.rbEvent = rbEvent;
rb.sendStrategy = sendStrategy;
rb._ = _;
...
rb.usePlugin = usePlugin;
rb.checkPluginInitStatus = checkPluginInitStatus;

rb.enableDataCollect = enableDataCollect;
...
rb.para = {
  ...
  batch_send: true,
  storage_store_key: 'rainbowdata2018_wechat',
  storage_prepare_data_key: 'rainbow_mp_prepare_data'
};
...
rb.setPara = function (para) {
  console.log(para)
  rb.para = _.extend2Lev(rb.para, para);
  console.log(rb.para)
  var channel = [];
  if (_.isArray(rb.para.source_channel)) {
    var len = rb.para.source_channel.length;
    var reserve_channel = ' utm_source utm_medium utm_campaign utm_content utm_term rb_utm ';
    for (var c = 0; c < len; c++) {
      if (reserve_channel.indexOf(' ' + rb.para.source_channel[c] + ' ') === -1) {
        channel.push(rb.para.source_channel[c]);
      }
    }
  }
  rb.para.source_channel = channel;

  if (_.isObject(rb.para.register)) {
    _.extend(_.info.properties, rb.para.register);
  }

  if (!rb.para.openid_url) {
    rb.para.openid_url = rb.para.server_url.replace(/([^\/])\/(rb)(\.gif){0,1}/, '$1/mp_login');
  }

  if (typeof rb.para.send_timeout !== 'number') {
    rb.para.send_timeout = 1000;
  }
  ...
  else if (rb.para.batch_send) {
    rb.para.datasend_timeout = 10000;
  }

  if (rb.para.batch_send === true) {
    rb.para.batch_send = _.extend({}, batch_send_default);
  } else if (_.isObject(rb.para.batch_send)) {
    rb.para.batch_send = _.extend({}, batch_send_default, rb.para.batch_send);
  }
  ...
  if (rb.para.is_persistent_save === true) {
    rb.para.is_persistent_save = _.extend({}, is_persistent_save_default);
    rb.para.is_persistent_save.utm = true;
  } else if (_.isObject(rb.para.is_persistent_save)) {
    rb.para.is_persistent_save = _.extend({}, is_persistent_save_default, rb.para.is_persistent_save);
  }
  ...
  rb.para.preset_properties = _.isObject(rb.para.preset_properties) ? rb.para.preset_properties : {};
};

rb.getServerUrl = function () {
  return rb.para.server_url;
};
...
var latest_source_channel = ['$latest_utm_source', '$latest_utm_medium', '$latest_utm_campaign', '$latest_utm_content', '$latest_utm_term', '$latest_rb_utm'];
var latest_share_info = ['$latest_share_distinct_id', '$latest_share_url_path', '$latest_share_depth', '$latest_share_method'];
var share_info_key = ['rainbow_share_d', 'rainbow_share_p', 'rainbow_share_i', 'rainbow_share_m'];
var page_show_time = Date.now();

var mpshow_time = null;
var mphide_time = null;
...
var wxSDKVersion = '';
rb.lib_version = LIB_VERSION;
rb.lib_name = LIB_NAME;
...
_.formatString = function (str) {
  if (str.length > rb.para.max_string_length) {
    logger.info('字符串长度超过限制，已经做截取--' + str);
    return str.slice(0, rb.para.max_string_length);
  } else {
    return str;
  }
};
...
_.strip_rb_properties = function (p) {
  if (!_.isObject(p)) {
    return p;
  }
...
_.getIsFirstDay = function () {
  if (typeof rb.store._state === 'object' && typeof rb.store._state.first_visit_day_time === 'number' && rb.store._state.first_visit_day_time > new Date().getTime()) {
    return true;
  } else {
    return false;
  }
...
_.getCurrentUrl = function (me) {
  ...
  var query = '';
  if (_.isObject(me) && me.rainbow_mp_encode_url_query) {
    query = me.rainbow_mp_encode_url_query;
  }
  if (path) {
      ...
      if (this.queue.length > 0) {
        _.each(this.queue, function (content) {
          if (content[0] === 'appLaunch') {
            rb.autoTrackCustom.appLaunch.apply(rb.autoTrackCustom, _.slice.call(content[1]));
          } else {
            rb[content[0]].apply(rb, _.slice.call(content[1]));
          }
        });
        this.queue = [];
      }
...
_.getCustomUtmFromQuery = function (query, utm_prefix, source_channel_prefix, rbutm_prefix) {
  if (!_.isObject(query)) {
    return {};
  }
  var result = {};
  if (query['rb_utm']) {
    for (var i in query) {
      if (i === 'rb_utm') {
        result[rbutm_prefix + i] = query[i];
        continue;
      }
      if (_.include(rb.para.source_channel, i)) {
...
_.removeStorageSync = function(storage_key){
    try {
		wx.removeStorageSync(storage_key);
	} catch (e) {
		// Do something when catch error
        logger.info('removeStorage fail')
	}
}
...
_.shareInfoData = function (para) {
  var shareData = {};
  var share = {};
  if (!rb.para.preset_events.share_info_use_string) {
    ...
    shareData = {
      depth: Number(share.rainbow_share_d),
      path: share.rainbow_share_p || '',
      id: share.rainbow_share_i || '',
      method: share.rainbow_share_m || ''
    };
  }
...
_.setShareInfo = function (para, prop) {
  var share = {};
  var obj = {};
  var current_id = rb.store.getDistinctId();
  var current_first_id = rb.store.getFirstId();
...
_.getShareInfo = function () {
  if (rb.para.preset_events.share_info_use_string) {
    var param = {
      rainbow_share_i: rb.store.getDistinctId() || '取值异常',
      rainbow_share_p: _.getCurrentPath(),
      rainbow_share_d: query_share_depth,
      rainbow_share_m: share_method
    };
...
_.detectOptionQuery = function (para) {
  ...
  function isBScene(obj) {
    var source = ['utm_source', 'utm_content', 'utm_medium', 'utm_campaign', 'utm_term', 'rb_utm'];
    var source_keyword = source.concat(rb.para.source_channel);
...
_.setRefPage = function () {
  ...
  try {
    ...
    if (pages && pages.route) {
      var url_query = pages.rainbow_mp_url_query ? '?' + pages.rainbow_mp_url_query : '';
...
_.getCurrentPageInfo = function () {
  ...
  if (pages && pages.route) {
    var query = pages.rainbow_mp_url_query ? '?' + pages.rainbow_mp_url_query : '';
...
_.wxrequest = function (obj) {
  if (_.compareSDKVersion(wxSDKVersion, '2.10.0') >= 0) {
    obj.timeout = rb.para.datasend_timeout;
    wx.request(obj);
  } else {
    ...
    function getSystemInfo() {
      wx.getSystemInfo({
        ...
        complete: function () {
          ...
          rb.initialState.systemIsComplete = true;
          rb.initialState.checkIsComplete();
        }
      });
    }
...
rb.eventSub = _.eventSub;

rb.events = new _.eventEmitter();
...
rb.store = {
  ...
  getStorage: function () {
    if (this.storageInfo) {
      ...
      this.storageInfo = rb._.getStorageSync(rb.para.storage_store_key) || '';
      return this.storageInfo;
    }
  },
  _state: {},
  ...
  toState: function (ds) {
    ...
    if (first_id) {
      if (old_login_id_name && this._state.identities.hasOwnProperty(old_login_id_name)) {
        ...
        this._state.identities[rb.para.login_id_key] = distinct_id;
        for (var identitiesprop in this._state.identities) {
          if (this._state.identities.hasOwnProperty(identitiesprop)) {
            if (identitiesprop !== '$identity_mp_id' && identitiesprop !== rb.para.login_id_key) {
              delete this._state.identities[identitiesprop];
            }
          }
        }
        this._state.history_login_id = {
          name: rb.para.login_id_key,
          value: distinct_id
        };
      }
    } else {
  ...
  set: function (name, value) {
    ...
    for (var i in obj) {
      ...
      if (i === 'first_id') {
        ...
        rb.events.emit('changeDistinctId');
      }
    }
    this.save();
  },
  identitiesSet: function (name, id) {
    ...
    switch (name) {
      case 'login':
        identities.$identity_mp_id = rb.store._state.identities.$identity_mp_id;
        identities[rb.para.login_id_key] = id;
        break;
      case 'logout':
        identities.$identity_mp_id = rb.store._state.identities.$identity_mp_id;
        break;
      case 'identify':
        identities = _.deepCopy(rb.store._state.identities);
        identities.$identity_anonymous_id = id;
        break;
    }
    rb.store.set('identities', identities);
  },
  ...
  encryptStorage: function () {
    ...
    rb._.setStorageSync(rb.para.storage_store_key, copyState);
  },
  save: function () {
...
rb.setOnceProfile = function (p, c) {
  rb.rbEvent.send({
...
rb.incrementProfile = function (p, c) {
  if (!_.isObject(p)) {
    return false;
  }
  var str = p;
...
rb.identify = function (id, isSave) {
  if (!checkPrivacyStatus()) {
    return false;
  }
  if (!meta.init_status) {
    rb.store.store_queue.push({
      method: 'identify',
      params: arguments
    });
    return false;
  ...
  id = _.validId(id);
  if (id) {
    var firstId = rb.store.getFirstId();
    if (isSave === true) {
      if (firstId) {
        rb.store.set('first_id', id);
      } else {
        rb.store.set('distinct_id', id);
      }
    } else {
      if (firstId) {
        rb.store.change('first_id', id);
      } else {
        rb.store.change('distinct_id', id);
      }
    }
    rb.store.identitiesSet('identify', id);
  }
};

rb.trackSignup = function (id, e, p, c) {
  var original_id = rb.store.getFirstId() || rb.store.getDistinctId();
  rb.store.set('distinct_id', id);
  rb.rbEvent.send({
...
rb.registerApp = function (obj) {
  if (_.isObject(obj) && !_.isEmptyObject(obj)) {
    _.info.currentProps = _.extend(_.info.currentProps, obj);
  }
};

rb.register = function (obj) {
  if (_.isObject(obj) && !_.isEmptyObject(obj)) {
    rb.store.setProps(obj);
  }
};

rb.clearAllRegister = function () {
  rb.store.setProps({}, true);
};

rb.clearAllProps = function (arr) {
  var obj = rb.store.getProps();
  var props = {};
  if (_.isArray(arr)) {
    _.each(obj, function (value, item) {
    ...
    rb.store.setProps(props, true);
  }
};

rb.clearAppRegister = function (arr) {
  if (_.isArray(arr)) {
...
_.setLatestChannel = function (channel) {
  if (!_.isEmptyObject(channel)) {
    if (includeChannel(channel, latest_source_channel)) {
      rb.clearAppRegister(latest_source_channel);
      rb.clearAllProps(latest_source_channel);
    }
    rb.para.is_persistent_save.utm ? rb.register(channel) : rb.registerApp(channel);
  }
...
_.setLatestShare = function (share) {
  if (share.$latest_share_depth || share.$latest_share_distinct_id || share.$latest_share_url_path || share.$latest_share_method) {
    rb.clearAppRegister(latest_share_info);
    rb.clearAllProps(latest_share_info);

    rb.para.is_persistent_save.share ? rb.register(share) : rb.registerApp(share);
  }
};
...
rb.login = function (id) {
  id = _.validId(id);
  if (id) {
    var firstId = rb.store.getFirstId();
    var distinctId = rb.store.getDistinctId();
    if (rb.store._state.identities.hasOwnProperty(rb.para.login_id_key) && id === firstId) {
      return false;
    }


    var isNewLoginId = !(rb.para.login_id_key === rb.store._state.history_login_id.name && rb.store._state.history_login_id.value === id);
    if (isNewLoginId) {
      rb.store._state.identities[rb.para.login_id_key] = id;
    }
    if (isNewLoginId) {
      if (firstId) {
        rb.trackSignup(id, '$SignUp');
      } else {
        rb.store.set('first_id', distinctId);
        rb.trackSignup(id, '$SignUp');
      }
    }
    if (isNewLoginId) {
      rb.store.identitiesSet('login', id);
      rb.store.set('history_login_id', {
        name: rb.para.login_id_key,
        value: id
      });
    }
...
rb.getAnonymousID = function () {
  if (_.isEmptyObject(rb.store._state)) {
    logger.info('请先初始化SDK');
  } else {
    return rb.store._state._first_id || rb.store._state.first_id || rb.store._state._distinct_id || rb.store._state.distinct_id;
  }
};

rb.logout = function (isChangeId) {
  var firstId = rb.store.getFirstId();
  if (firstId) {
    rb.store.set('first_id', '');
    if (isChangeId === true) {
      rb.store.set('distinct_id', rb.store.getUUID());
    } else {
      rb.store.set('distinct_id', firstId);
    }
  } else {
    logger.info('没有first_id，logout失败');
  }
  rb.store.identitiesSet('logout');
  rb.store.set('history_login_id', {
    name: '',
    value: ''
  });
...
rb.getLocation = function () {
  wx.getSetting({
    success: function (res) {
      if (res.authSetting['scope.userLocation']) {
        wx.getLocation({
          type: rb.para.preset_properties.location.type,
          success: function (res) {
            rb.registerApp({
              $latitude: res.latitude * Math.pow(10, 6),
              $longitude: res.longitude * Math.pow(10, 6),
              $geo_coordinate_system: _.setUpperCase(rb.para.preset_properties.location.type)
            });
          },
          fail: function (err) {
...
rb.openid = {
  getRequest: function (callback) {
    wx.login({
      success: function (res) {
        console.log(2)
        if (res.code && rb.para.appid && rb.para.openid_url) {
          _.wxrequest({
            url: rb.para.openid_url + '&code=' + res.code + '&appid=' + rb.para.appid,
            method: 'GET',
  ...
  getWXStorage: function () {
    var storageInfo = rb.store.getStorage();
...
rb.init = function (obj) {
  ...
  rb.checkPluginInitStatus();
  if (rb.para.batch_send) {
    sendStrategy.init();
  }
  rb.initialState.storeIsComplete = true;
  rb.initialState.checkIsComplete();
  checkAppLaunch();
};

rb.getPresetProperties = function () {
  if (_.info && _.info.properties && _.info.properties.$lib) {
    ...
    var obj = _.extend(builtinProps, propertyObj, _.info.properties, rb.store.getProps());
    delete obj.$lib;
    return obj;
  } else {
    return {};
...
rb.setOpenid = function (openid, isCover) {
  ...
  rb.store.set('openid', openid);
  rb.identify(openid, true);

  var name = _.getOpenidNameByAppid();
  rb.store._state.identities[name] = openid;
  rb.store.save();
};

rb.unsetOpenid = function (val) {
  var id = _.validId(val);
  if (!id) {
    return false;
  }
  var openid = rb.store._state.openid;
  if (openid === id) {
    rb.store.set('openid', '');
  }

  var name = _.getOpenidNameByAppid();
  if (rb.store._state.identities.hasOwnProperty(name) && id === rb.store._state.identities[name]) {
    delete rb.store._state.identities[name];
    rb.store.save();
  }
};

rb.setUnionid = function (val) {
  var id = _.validId(val);
  if (id) {
    rb.store._state.identities['$identity_mp_unionid'] = id;
    rb.store.save();
  }
};

rb.unsetUnionid = function (val) {
  var id = _.validId(val);
  if (id) {
    if (rb.store._state.identities.hasOwnProperty('$identity_mp_unionid') && id === rb.store._state.identities['$identity_mp_unionid']) {
      var openid = _.getOpenidNameByAppid();
      if (rb.store._state.identities.hasOwnProperty(openid)) {
        delete rb.store._state.identities[openid];
        delete rb.store._state.openid;
        rb.store.save();
      }
      delete rb.store._state.identities['$identity_mp_unionid'];
      rb.store.save();
    }
  }
};

rb.initWithOpenid = function (options, callback) {
  options = options || {};
  if (options.appid) {
    rb.para.appid = options.appid;
  }
  rb.openid.getOpenid(function (openid) {
    if (openid) {
      rb.setOpenid(openid, options.isCoverLogin);
    }
...
rb.bind = function (name, value) {
  ...
  if (!_.check.checkKeyword(name) || name === '$identity_anonymous_id' || name === '$mp_id' || name === '$identity_mp_id' || name === '$mp_openid' || name === '$identity_mp_openid' || name === '$identity_mp_unionid' || name === '$mp_unionid' || name === '$identity_login_id' || name === rb.para.login_id_key) {
  ...
  var identities = rb.store._state.identities;
...
rb.unbind = function (name, value) {
  ...
  if (rb.store._state.identities.hasOwnProperty(name) && value === rb.store._state.identities[name]) {
    delete rb.store._state.identities[name];
    rb.store.save();
  }
  var para = {};
...
rb.setWebViewUrl = function (url, after_hash) {
  ...
  var distinct_id = rb.store.getDistinctId() || '',
    first_id = rb.store.getFirstId() || '',
    idIndex;
  ...
  if (after_hash) {
    idIndex = hash.indexOf('_rbsdk');
    var queryIndex = hash.indexOf('?');
    if (queryIndex > -1) {
      if (idIndex > -1) {
        nurl = host + search + '#' + hash.substring(1, idIndex) + '_rbsdk=' + value;
      } else {
        nurl = host + search + '#' + hash.substring(1) + '&_rbsdk=' + value;
      }
    } else {
      nurl = host + search + '#' + hash.substring(1) + '?_rbsdk=' + value;
    }
  } else {
    idIndex = search.indexOf('_rbsdk');
    var hasQuery = /^\?(\w)+/.test(search);
    if (hasQuery) {
      if (idIndex > -1) {
        var newSearch = search.replace(/(_rbsdk=)([^&]*)/gi, '_rbsdk=' + value);
        nurl = host + newSearch + hash;
      } else {
        nurl = host + '?' + search.substring(1) + '&_rbsdk=' + value + hash;
      }
    } else {
      nurl = host + '?' + search.substring(1) + '_rbsdk=' + value + hash;
    }
...
_.each(['setProfile', 'setOnceProfile', 'track', 'quick', 'incrementProfile', 'appendProfile', 'login', 'logout', 'registerApp', 'register', 'clearAllRegister', 'clearAllProps', 'clearAppRegister', 'bind', 'unbind', 'unsetOpenid', 'setUnionid', 'unsetUnionid'], function (method) {
  ...
  rb[method] = function () {
    ...
    if (rb.initialState.isComplete) {
      temp.apply(rb, arguments);
    } else {
      rb.initialState.queue.push([method, arguments]);
    }
  };
});
...
_.sendPageLeave = function () {
  ...
  if (page_show_time >= 0 && router !== '') {
    ...
    prop.$url_query = currentPage.rainbow_mp_url_query ? currentPage.rainbow_mp_url_query : '';
    prop.$url_path = router;
    prop.$title = title;
    prop.event_duration = page_stay_time;
    rb.track('$MPPageLeave', prop);
    page_show_time = -1;
  }
};
...
rb.autoTrackCustom = {
  trackCustom: function (api, prop, event) {
    var temp = rb.para.autoTrack[api];
    var tempFunc = '';
    if (rb.para.autoTrack && temp) {
  ...
  appLaunch: function (para, not_use_auto_track) {
    if (typeof this === 'object' && !this['trackCustom']) {
      this[rb.para.name] = rb;
    }
    if (!checkPrivacyStatus()) {
      return false;
    }
    if (!rb.initialState.isComplete) {
      rb.initialState.queue.push(['appLaunch', arguments]);
    ...
    if (para && para.scene) {
      ...
      rb.current_scene = current_scene;
    ...
    if (not_use_auto_track) {
      ...
      rb.track('$MPLaunch', prop);
    } else if (rb.para.autoTrack && rb.para.autoTrack.appLaunch) {
      rb.autoTrackCustom.trackCustom('appLaunch', prop, '$MPLaunch');
    }
  },
  appShow: function (para, not_use_auto_track) {
    ...
    if (_.isObject(rb.para.preset_properties.location) && (rb.para.preset_properties.location.type === 'wgs84' || rb.para.preset_properties.location.type === 'gcj02')) {
      rb.getLocation();
    }
    _.setShareInfo(para, prop);
    ...
    var session_expired_time = rb.para.session_expired_time || 30;
    if (mphide_time && (mpshow_time - mphide_time) / 1000 > session_expired_time) {
        rb.track('$MPEnd',prop)
        var session_id = Math.random();
        rb.registerApp({
            session_id: session_id,
    ...
    if (not_use_auto_track) {
      prop = _.extend(prop, not_use_auto_track);
      rb.track('$MPShow', prop);
    } else if (rb.para.autoTrack && rb.para.autoTrack.appShow) {
      rb.autoTrackCustom.trackCustom('appShow', prop, '$MPShow');
    }
  },
  appHide: function (not_use_auto_track) {
    ...
    _.setPageRefData(prop);
    mphide_time = current_time;
    if (not_use_auto_track) {
      prop = _.extend(prop, not_use_auto_track);
      rb.track('$MPHide', prop);
    } else if (rb.para.autoTrack && rb.para.autoTrack.appHide) {
      rb.autoTrackCustom.trackCustom('appHide', prop, '$MPHide');
    }
  ...
  pageLoad: function (para) {
    ...
    if (para && _.isObject(para)) {
      this.rainbow_mp_url_query = _.setQuery(para);
      this.rainbow_mp_encode_url_query = _.setQuery(para, true);
    }
  },
  pageShow: function () {
    ...
    _.setRefPage();
    prop.$url_path = router;
    prop.$url_query = this.rainbow_mp_url_query ? this.rainbow_mp_url_query : '';
    ...
    if (rb.para.onshow) {
      rb.para.onshow(rb, router, this);
    } else if (!(_.isObject(rb.para.autotrack_exclude_page) && _.isArray(rb.para.autotrack_exclude_page.pageShow) && rb.para.autotrack_exclude_page.pageShow.indexOf(router) !== -1)) {
      rb.autoTrackCustom.trackCustom('pageShow', prop, '$MPViewScreen');
    }
  },
  pageShare: function (option) {
    ...
    option.onShareAppMessage = function () {
      ...
      if (rb.para.autoTrack && rb.para.autoTrack.pageShare) {
        var prop = {
          $url_path: _.getCurrentPath(),
        ...
        _.setPageRefData(prop);
        rb.autoTrackCustom.trackCustom('pageShare', prop, '$MPShare');
      }

      if (rb.para.allow_amend_share_path) {
  ...
  pageAddFavorites: function () {
    var prop = {};
    prop.$url_path = _.getCurrentPath();
    if (rb.para.autoTrack && rb.para.autoTrack.mpFavorite) {
      rb.autoTrackCustom.trackCustom('mpFavorite', prop, '$MPAddFavorites');
    }
  }
};
...
rb.quick = function () {
  ...
  if (arg0 === 'getAnonymousID') {
    ...
    if (arg1) {
      rb.autoTrackCustom[arg0](arg1, prop);
    } else {
      logger.info('App的launch和show，在rainbow.quick第二个参数必须传入App的options参数');
    }
  } else if (arg0 === 'appHide') {
    prop = _.isObject(arg1) ? arg1 : {};
    rb.autoTrackCustom[arg0](prop);
  }
};
rb.appLaunch = function (option, prop) {
  var obj = {};
  ...
  rb.track('$MPLaunch', obj);
};
rb.appShow = function (option, prop) {
  var obj = {};
  mpshow_time = new Date().getTime();
  if (option && option.scene) {
    current_scene = option.scene;
  ...
  var session_expired_time = rb.para.session_expired_time || 30;
  if (new Boolean(mphide_time) && (mpshow_time - mphide_time) / 1000 > session_expired_time) {
		var session_id = Math.random();
		rb.registerApp({
			session_id: session_id,
		});
...
rb.pageShow = function (prop) {
  ...
  obj.$url_path = router;
  obj.$url_query = currentPage.rainbow_mp_url_query ? currentPage.rainbow_mp_url_query : '';
...
if (global && global.rainbow_data_pre_config) {
  var name = global.rainbow_data_pre_config.login_id_key;

...
function encodeTrackData(data) {
  ...
  var dataStr = sa._.urlBase64Encode(data);
...
var sendStrategy = {
  ...
  wxrequest: function (option) {
    if (sa._.isArray(option.data) && option.data.length > 0) {
      ...
      var app_id = sa.para.app_id;
      var app_secret = sa.para.app_secret;
...
function onceSend(data) {
  ...
  sa._.wxrequest({
    url: sa.para.server_url + '?data=' + encodeData + '&app_id=' + app_id,
    method: 'GET',
    header: {
      timestamp: timestamp,
      signature: _.md5(dataStr),
...
sa.autoTrackCustom = {
  ...
  appShow: function (para, not_use_auto_track) {
    ...
    var session_expired_time = sa.para.session_expired_time || 30;
